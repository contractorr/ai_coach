...
def test_projects_discover_uses_shared_service(runner):
    ...
    with patch('cli.commands.projects.get_components', return_value=components), patch(
        'cli.commands.projects.get_profile_storage', return_value=profile_storage
    ), patch(
    ...
    mock_discover.assert_called_once_with(components['intel_storage'], profile=profile_storage.load.return_value, limit=5, days=7)
...
def test_projects_list_uses_shared_list_service(runner):
    ...
    with patch('cli.commands.projects.get_components', return_value=components), patch(
        ...
        return_value={"issues": [{"title": "Fix auth bug", "summary": "Short summary", "url": "https://github.com/acme/app/issues/1"}], "count": 1},
    ) as mock_list:
